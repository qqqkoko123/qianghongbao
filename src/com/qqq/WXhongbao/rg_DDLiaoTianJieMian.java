...
public class rg_DDLiaoTianJieMian extends rg_RenWuJiChuLei {
    ...
    public void rg_ShouDaoXinShiJian1 (android.view.accessibility.AccessibilityEvent rg_ShiJianDuiXiang69) {
        super.rg_ShouDaoXinShiJian1 (rg_ShiJianDuiXiang69);
        if (rg_QiYongZhuangTai3 == false)
        {
            return;
        }
        if (huoShan.Java.JiBen.rg_WenBenXingLei.rg_WenBenXiangDeng (String.valueOf(rg_ShiJianDuiXiang69.getPackageName()), "com.alibaba.android.rimet", false) == false)
        {
            return;
        }
        java.util.List<android.view.accessibility.AccessibilityNodeInfo> rg_JieDianLieBiao2;
        android.view.accessibility.AccessibilityNodeInfo rg_JieDian;
        int rg_ChengYuanShu26;
        rg_JieDianLieBiao2 = rg_ZhiDingIDChaZhaoJieDianLieBiao1 ("com.alibaba.android.rimet:id/chatting_content_view_stub");
        rg_ChengYuanShu26 = rg_JieDianLieBiao2.size ();
        if (rg_JieDianLieBiao2 != null && rg_JieDianLieBiao2.size () > 0)
        {
            rg_JieDian = rg_JieDianLieBiao2.get (rg_QuanJuBianLiangLei.rg_ChengYuanSuoYindd);
            if (rg_JieDian != null)
            {
                if (rg_QuanJuBianLiangLei.rg_ChengYuanSuoYindd <= rg_ChengYuanShu26 - 2)
                {
                ...
                if (rg_JieDian.getChildCount() == 3)
                {
                    if ((String.valueOf(huoShan.AnZhuo.GongNeng.MoZhangAiFuWu.rg_MoZhangAiJieDianXinXiLei.rg_QuZhiDingZiJieDian (rg_JieDian, 2).getText()).contains ("已领取")) == false)
                    {
                        rg_ChanJiJieDian1 (rg_JieDian);
                    }
                }
